...
import { Camera, User, Eye, EyeOff, Mail, Phone, Building } from 'lucide-react';
...
const Profile: React.FC = () => {
  ...
  const [profile, setProfile] = useState<MarketerProfile>(() => {
    const saved = localStorage.getItem('marketerProfile');
  ...
  useEffect(() => {
    localStorage.setItem('marketerProfile', JSON.stringify(profile));
  }, [profile]);
  ...
  const handleSaveImage = async () => {
    if (imageRef.current && previewImage) {
      try {
        ...
        const marketerIndex = marketers.findIndex((m: any) => m.id === user?.id);

        if (marketerIndex !== -1) {
          marketers[marketerIndex].profilePicture = croppedImageUrl;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Update marketer data in localStorage
    const storedMarketers = localStorage.getItem('marketers');
    const marketers = storedMarketers ? JSON.parse(storedMarketers) : [];
    const marketerIndex = marketers.findIndex((m: any) => m.id === user?.id);
          ...
          <Card className="bg-white/5 backdrop-blur-sm border-t-2 border-t-purple-500 border-x-0 border-b-0 rounded-lg overflow-hidden">
            ...
            <CardContent className="mt-6">
              <div className="flex items-center space-x-4 mb-8">
                ...
                <div>
                  <p className="text-white/90 text-lg">{user?.role || 'marketer'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="block text-sm text-white/60">Company</label>
                  <Input
                    type="text"
